...
# Original file found at https://github.com/concourse/concourse-docker/blob/master/generate-keys.sh
...
yes | sudo ssh-keygen $PEM_OPTION -t rsa -f ./keys/web/tsa_host_key -N ''
yes | sudo ssh-keygen $PEM_OPTION -t rsa -f ./keys/web/session_signing_key -N ''

yes | sudo ssh-keygen $PEM_OPTION -t rsa -f ./keys/worker/worker_key -N ''
